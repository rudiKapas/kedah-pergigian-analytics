(function () {
  ...
  function cell(arr, addr) {
    const m = /^([A-Z]+)(\d+)$/.exec(addr);
    if (!m) return 0;
    const r = parseInt(m[2], 10) - 1;
    const c = colIndexFromLetter(m[1]);
    return cleanInt((arr[r] || [])[c]);
  }
  ...
  const MAP1 = [
    { name: "Kota Setar", col: "C" }, { name: "Pendang", col: "D" }, { name: "Kuala Muda", col: "E" },
    { name: "Sik", col: "F" }, { name: "Kulim", col: "G" }, { name: "Bandar Baru", col: "H" },
    { name: "Kubang Pasu", col: "I" }, { name: "Padang Terap", col: "J" }, { name: "Baling", col: "K" },
    { name: "Yan", col: "L" }, { name: "Langkawi", col: "M" }, { name: "Kedah", col: "N" }
  ];
  ...
  function cleanPct(x) {
    ...
    if (!had && v > 0 && v <= 1) v = v * 100;
    return +v.toFixed(2);
  }
  function padEnds(labels, series) {
    return { labels: ["", ...labels, ""], series: [0, ...series.map(v => v ?? 0), 0] };
  }
  function draw1(rows, canvasId, mode) {
    ...
    const chart = new Chart(ctx, {
      ...
      data: {
        labels: X,
        datasets: [
          { label: "% Menerima Perkhidmatan", data: pA.series, borderColor: "#f59e0b", backgroundColor: g1, borderWidth: 3, tension: 0.45, fill: true, spanGaps: true, pointRadius: (c) => (c.dataIndex === 0 || c.dataIndex === X.length - 1) ? 0 : 3, yAxisID: "y1" },
          { label: "Anggaran Penduduk", data: pP.series, borderColor: "#6366f1", backgroundColor: g2, borderWidth: 3, tension: 0.45, fill: true, spanGaps: true, pointRadius: (c) => (c.dataIndex === 0 || c.dataIndex === X.length - 1) ? 0 : 3, yAxisID: "y2" }
        ]
      },
      options: {
        responsive: true, maintainAspectRatio: false, layout: { padding: { bottom: 10 } },
        plugins: { legend: { display: false }, tooltip: { mode: "index", intersect: false, filter: (i) => !(i.dataIndex === 0 || i.dataIndex === X.length - 1), callbacks: { label: (c) => c.datasetIndex === 0 ? ` Akses: ${c.parsed.y || 0}%` : ` Populasi: ${niceNum(c.parsed.y)}` } } },
        scales: {
    ...
    if (canvasId === "infogChart") CH1 = chart;
    return chart;
  }
  async function load1() {
    ...
    try {
      ...
      const popR = RAW1[9] || [];
      const accR = RAW1[10] || [];
      const rows = MAP1.map(m => {
        const i = colIndexFromLetter(m.col);
        const population = Number(String(popR[i] ?? "").replace(/[^\d]/g, "")) || null;
        let access = cleanPct(accR[i]);
        if (access === null && String(accR[i] ?? "").trim() === "0") access = 0;
        return { name: m.name, population, access };
      });
  ...
  function compute2(arr, keys) {
    const labels = ["", ...DIST2.map(d => d.name), ""];
    const per = [];
    CATS2.forEach(c => {
      if (!keys.has(c.key)) return;
      const b = [0], u = [0];
      DIST2.forEach(d => { b.push(sumCells(arr, d.col, c.b)); u.push(sumCells(arr, d.col, c.u)); });
      b.push(0); u.push(0);
      per.push({ key: c.key, baru: b, ulangan: u });
    });
    return { labels, per };
  }
  function draw2(data, canvasId, mode) {
    const ctx = $(canvasId).getContext("2d");
    if (CH2 && canvasId === "chartPrimer") CH2.destroy();
    const sets = [];
    data.per.forEach(c => {
      const color = CAT_COLOR2[c.key] || "#64748b";
      sets.push({ label: c.key + " • Baru", data: c.baru, borderColor: color, backgroundColor: "transparent", borderWidth: 3, tension: 0.45, fill: false });
      sets.push({ label: c.key + " • Ulangan", data: c.ulangan, borderColor: color, backgroundColor: "transparent", borderWidth: 3, tension: 0.45, fill: false, borderDash: [6, 4] });
    });
    const chart = new Chart(ctx, {
      type: "line",
    ...
    if (canvasId === "chartPrimer") CH2 = chart;
    return chart;
  }
  async function load2() {
    ...
    try {
      ...
      const d = compute2(RAW2, selected2());
      draw2(d, "chartPrimer", "main");
      $("ddBtn2").onclick = () => $("ddMenu2").classList.toggle("open");
      $("ddMenu2").querySelectorAll("input[type=checkbox]").forEach(i => {
        i.addEventListener("change", () => { tagsLegend2(); const d2 = compute2(RAW2, selected2()); draw2(d2, "chartPrimer", "main"); });
      });
      ...
      console.error("Tile 2 error:", e); err.style.display = "block"; err.textContent = "Gagal memuatkan CSV (Tile 2).";
    }
  }
  ...
  function compute3(arr, keys) {
    const labels = ["", ...DIST3.map(d => d.name), ""];
    const per = [];
    SERVICES.forEach(s => {
      if (!keys.has(s.key)) return;
      const b = [0], u = [0];
      DIST3.forEach(d => { b.push(cell(arr, d.col + String(s.baru))); u.push(cell(arr, d.col + String(s.ulangan))); });
      b.push(0); u.push(0);
      per.push({ key: s.key, color: s.color, baru: b, ulangan: u });
    });
    return { labels, per };
  }
  function draw3(data, canvasId, mode) {
    const ctx = $(canvasId).getContext("2d");
    if (CH3 && canvasId === "chartOutreach") CH3.destroy();
    const sets = [];
    data.per.forEach(s => {
      const color = s.color || "#64748b";
      sets.push({ label: s.key + " • Baru", data: s.baru, borderColor: color, backgroundColor: "transparent", borderWidth: 3, tension: 0.45, fill: false });
      sets.push({ label: s.key + " • Ulangan", data: s.ulangan, borderColor: color, backgroundColor: "transparent", borderWidth: 3, tension: 0.45, fill: false, borderDash: [6, 4] });
    });
    const chart = new Chart(ctx, {
      type: "line",
      data: { labels: data.labels, datasets: sets },
    ...
    if (canvasId === "chartOutreach") CH3 = chart;
    return chart;
  }
  async function load3() {
    ...
    try {
      ...
      const d = compute3(RAW3, selected3());
      draw3(d, "chartOutreach", "main");
      $("ddBtn3").onclick = () => $("ddMenu3").classList.toggle("open");
      $("ddMenu3").querySelectorAll("input[type=checkbox]").forEach(i => {
        i.addEventListener("change", () => { tagsLegend3(); const d2 = compute3(RAW3, selected3()); draw3(d2, "chartOutreach", "main"); });
      });
      ...
      console.error("Tile 3 error:", e); err.style.display = "block"; err.textContent = "Gagal memuatkan CSV (Tile 3).";
    }
  }
  ...
  function compute4(arr, keys) {
    const labels = ["", ...DIST4.map(d => d.name), ""];
    const per = [];
  ...
  function draw4(data, canvasId, mode) {
    const ctx = $(canvasId).getContext("2d");
