(function () {
  ...
  function cleanPct(v){
    ...
    const had=s.includes("%"); s=s.replace(/[% ]/g,"").replace(/,/g,".");
  ...
  async function fetchCSV(url){
    ...
    for(const u of tries){
      try{ const r=await fetch(u,{mode:"cors",cache:"no-store"}); if(!r.ok) throw 0; const t=await r.text(); if(t&&t.length>10) return t; }
  ...
  let MCH=null;
  function openModal(t){ mtitle.textContent=t||"Perincian"; modal.classList.add("open"); document.documentElement.style.overflow="hidden"; document.body.style.overflow="hidden"; }
  ...
  function drawT1(rows, canvas, mode){
    ...
    CH1=new Chart(ctx,{
      ...
      options:{
        ...
        scales:{
          x:{grid:{display:false},ticks:{autoSkip:false,maxRotation:mode==="main"?90:40,minRotation:mode==="main"?90:40,
  ...
  function buildDD(menuId, btnAll, btnNone, btnClose, items, def){
    ...
    items.forEach((txt,i)=>{ const el=document.createElement("label"); el.className="row"; const ck=(def?txt===def:i===0)?"checked":""; el.innerHTML=`<input type="checkbox" data-k="${txt}" ${ck}> ${txt}`; frag.appendChild(el); });
  ...
  function computeT2(arr, keys){
    ...
    CATS2.forEach(c=>{ if(!keys.has(c.key)) return; const b=[0], u=[0];
      DIST2.forEach(d=>{ b.push(sumCol(arr,d.L,c.b)); u.push(sumCol(arr,d.L,c.u)); });
      b.push(0); u.push(0); per.push({key:c.key,b,u}); });
    return { labels, per };
  }
  function drawT2(data, canvas, mode){
    ...
    CH2=new Chart($(canvas).getContext("2d"),{
      ...
      options:{responsive:true,maintainAspectRatio:false,
        ...
        scales:{x:{grid:{display:false},ticks:{autoSkip:false,maxRotation:mode==="main"?90:40,minRotation:mode==="main"?90:40,callback:(v,i)=>(i===0||i===data.labels.length-1)?"":data.labels[i]}},
  ...
  function computeT3(arr,set){
    ...
    SVCS.forEach(s=>{ if(!set.has(s.key)) return; const b=[0], u=[0];
      DIST3.forEach(d=>{ b.push(cell(arr,d.L+String(s.b))); u.push(cell(arr,d.L+String(s.u))); });
      b.push(0); u.push(0); per.push({key:s.key,color:s.color,b,u}); });
    return {labels, per};
  }
  ...
  function computeT4(arr,set){
    ...
    MET.forEach(m=>{ if(!set.has(m.key)) return; const s=[0]; DIST4.forEach(d=>s.push(cell(arr,d.L+String(m.row)))); s.push(0); per.push({key:m.key,type:m.type,color:m.color,target:m.target,data:s}); });
    return { labels, per };
  }
  ...
  function drawT4(data,canvas,mode){
    ...
    data.per.forEach(m=>{
      ...
      d.yAxisID=m.type==="cnt"?"yR":"yL"; if(m.key.includes("Lift")) d.borderDash=[6,4];
      ds.push(d);
      if(m.target!=null && m.type!=="cnt"){
        const t=addTargetLine(m.data.length, m.target);
        ds.push({label:`Sasaran ${m.key.split("%").pop().trim()}`,data:t.data,borderColor:t.color,borderWidth:2,borderDash:[4,4],pointRadius:0,fill:false,yAxisID:"yL"});
      }
    });
  ...
  async function loadT4(){
    ...
    try{
      buildDD4(); const csv=await fetchCSV(CSV4); RAW4=Papa.parse(csv,{header:false,skipEmptyLines:true}).data;
  ...
  // =======================================================
  // TILE 5 — Ibu Mengandung (3 metrik + sasaran)
  // =======================================================
  ...
  // ---------- light reflow on resize ----------
  window.addEventListener("resize", ()=>{
    if(RAW1){ const popRow=RAW1[9]||[], accRow=RAW1[10]||[]; drawT1(DIST1.map(d=>{const i=colIdx(d.L);return {n:d.n,a:cleanPct(accRow[i])||0,p:cleanInt(popRow[i])};}),"t1","main"); }
