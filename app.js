...
/* dropdowns triggered by icon-chip */
...
function chosen(menuId){
  const set = new Set();
  qsa("#"+menuId+" .menu-body input[type=checkbox]").forEach(function(cb){
...
function wireDropdown(boxId){
  ...
  if (allBtn)  allBtn.addEventListener("click", function(){ qsa(".menu-body input", menu).forEach(function(c){ c.checked=true; }); });
  if (noneBtn) noneBtn.addEventListener("click", function(){ qsa(".menu-body input", menu).forEach(function(c){ c.checked=false; }); });
...
function makeArea(color, fill, y, w){
  return { borderColor:color, backgroundColor:fill, borderWidth:w||3, tension:0.45, fill:true, yAxisID:y||"y" };
}
...
function allZero(arr){
  return arr.every(function(v){ return (Array.isArray(v)?allZero(v):(Number(v)||0)===0); });
}
...
/* ===== TILE 7 – Young Adult ===== */
var CSV_YA = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSS9NxgDwQDoJrQZJS4apFq-p5oyK3B0WAnFTlCY2WGcvsMzNBGIZjilIez1AXWvAIZgKltIxLEPTFT/pub?gid=543945307&single=true&output=csv";
var YA_MET = [
  {k:"Peratus tidak perlu rawatan", row:8,  target:70, color:"#0ea5e9"},
  {k:"Peratus kes selesai (Orally Fit)", row:14, target:70, color:"#10b981"}
];
var RAW_YA=null, CH_YA=null;

function computeYA(arr, set){
  var labels = [""].concat(DIST.map(function(d){return d.n;})).concat([""]);
  var series = [];
  YA_MET.forEach(function(m){
    if (!set.has(m.k)) return;
    var data = [0];
    DIST.forEach(function(d){ data.push( valPct(a1Cell(arr, d.L+String(m.row))) ); });
    data.push(0);
...
function drawYA(data, canvasId){
  ...
  var g1 = ctx.createLinearGradient(0,0,0,260); g1.addColorStop(0,"rgba(14,165,233,.35)"); g1.addColorStop(1,"rgba(14,165,233,.04)");
  var g2 = ctx.createLinearGradient(0,0,0,260); g2.addColorStop(0,"rgba(16,185,129,.35)"); g2.addColorStop(1,"rgba(16,185,129,.04)");
  var datasets=[];
  ...
  CH_YA = new Chart(ctx,{ type:"line", data:{ labels:data.labels, datasets:datasets }, options:{
    ...
    plugins:{ legend:{display:false},
      tooltip:{ mode:"index", intersect:false,
        filter:function(i){ return !(i.dataIndex===0||i.dataIndex===data.labels.length-1); } } },
    scales:{
      x:{ grid:{display:false}, ticks:{ autoSkip:false, maxRotation:90, minRotation:90,
        callback:function(v,i){ return (i===0||i===data.labels.length-1)?"":data.labels[i]; } } },
      yL:{ position:"left", beginAtZero:true, ticks:{ callback:function(v){ return v+"%"; } } }
    }
  }});
  var flat=[];
  datasets.forEach(function(d){ if(Array.isArray(d.data)) flat=flat.concat(d.data.slice(1,-1)); });
  if (allZero(flat)){ ctx.font="12px Inter"; ctx.fillStyle="#94a3b8"; ctx.fillText("Tiada data untuk dipaparkan",12,22); }
}
function buildDD7(){ buildDropdown("dd7menu", YA_MET.map(function(m){return m.k;}), YA_MET[0].k); wireDropdown("dd7"); }
function refreshYA(){
  var s = chosen("dd7menu");
  var tags = $("dd7tags"); tags.innerHTML="";
  s.forEach(function(k){ var t=document.createElement("span"); t.className="tag"; t.textContent=k; tags.appendChild(t); });
  if (RAW_YA) drawYA(computeYA(RAW_YA,s),"t7");
}
async function loadYA(){
  $("t7err").style.display="none";
  try{
    var csv = await fetchCSV(CSV_YA);
    RAW_YA = Papa.parse(csv,{header:false,skipEmptyLines:true}).data;
    $("t7time").textContent = new Date().toLocaleString();
    buildDD7(); refreshYA();
    qsa("#dd7menu input").forEach(function(i){ i.addEventListener("change", refreshYA); });
  }catch(e){
    console.error(e); $("t7err").textContent="Gagal memuatkan CSV (YA)."; $("t7err").style.display="block";
  }
}
$("t7refresh").addEventListener("click", loadYA);
$("t7expand").addEventListener("click", function(){
  if (!RAW_YA) return;
...
/* ===== TILE 8 – BPE ===== */
var CSV_BPE = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSS9NxgDwQDoJrQZJS4apFq-p5oyK3B0WAnFTlCY2WGcvsMzNBGIZjilIez1AXWvAIZgKltIxLEPTFT/pub?gid=1983552555&single=true&output=csv";
var BPE_MET = [
  {k:"Peratus BPE screening", row:8, color:"#0ea5e9"},
  {k:"Peratus BPE = 0",       row:13,color:"#8b5cf6"}
];
var RAW_BPE=null, CH_BPE=null;

function computeBPE(arr, set){
  var labels = [""].concat(DIST.map(function(d){return d.n;})).concat([""]);
  var series = [];
  BPE_MET.forEach(function(m){
    if (!set.has(m.k)) return;
    var data=[0];
    DIST.forEach(function(d){ data.push( valPct(a1Cell(arr, d.L+String(m.row))) ); });
    data.push(0);
...
function drawBPE(data, canvasId){
  ...
  var ctx = $(canvasId).getContext("2d");
  var g1 = ctx.createLinearGradient(0,0,0,260); g1.addColorStop(0,"rgba(14,165,233,.35)"); g1.addColorStop(1,"rgba(14,165,233,.04)");
  var g2 = ctx.createLinearGradient(0,0,0,260); g2.addColorStop(0,"rgba(139,92,246,.35)"); g2.addColorStop(1,"rgba(139,92,246,.04)");
  var datasets = data.series.map(function(s,i){
    var fill = (i===0)?g1:g2;
    var cfg  = makeArea(s.color, fill, "yL");
...
function buildDD8(){ buildDropdown("dd8menu", BPE_MET.map(function(m){return m.k;}), BPE_MET[0].k); wireDropdown("dd8"); }
function refreshBPE(){
  var s = chosen("dd8menu");
  var tags = $("dd8tags"); tags.innerHTML="";
  s.forEach(function(k){ var t=document.createElement("span"); t.className="tag"; t.textContent=k; tags.appendChild(t); });
...
async function loadBPE(){
  ...
  try{
    var csv = await fetchCSV(CSV_BPE);
    RAW_BPE = Papa.parse(csv,{header:false,skipEmptyLines:true}).data;
    $("t8time").textContent = new Date().toLocaleString();
    buildDD8(); refreshBPE();
    qsa("#dd8menu input").forEach(function(i){ i.addEventListener("change", refreshBPE); });
  }catch(e){
    console.error(e); $("t8err").textContent="Gagal memuatkan CSV (BPE)."; $("t8err").style.display="block";
  }
}
$("t8refresh").addEventListener("click", loadBPE);
$("t8expand").addEventListener("click", function(){
  if (!RAW_BPE) return;
...
/* ===== TILE 9 – Warga Emas ===== */
var CSV_WE = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSS9NxgDwQDoJrQZJS4apFq-p5oyK3B0WAnFTlCY2WGcvsMzNBGIZjilIez1AXWvAIZgKltIxLEPTFT/pub?gid=480846724&single=true&output=csv";
var WE_MET = [
...
var RAW_WE=null, CH_WE=null;

function computeWE(arr, set){
  var labels = [""].concat(DIST.map(function(d){return d.n;})).concat([""]);
  var series = [];
  WE_MET.forEach(function(m){
    if (!set.has(m.k)) return;
    var data=[0];
    DIST.forEach(function(d){
      var A1 = d.L+String(m.row);
...
function drawWE(data, canvasId){
  ...
  var ctx = $(canvasId).getContext("2d");
  var gA = ctx.createLinearGradient(0,0,0,260); gA.addColorStop(0,"rgba(14,165,233,.35)"); gA.addColorStop(1,"rgba(14,165,233,.04)");
  var gB = ctx.createLinearGradient(0,0,0,260); gB.addColorStop(0,"rgba(16,185,129,.35)"); gB.addColorStop(1,"rgba(16,185,129,.04)");
  var gC = ctx.createLinearGradient(0,0,0,260); gC.addColorStop(0,"rgba(245,158,11,.35)"); gC.addColorStop(1,"rgba(245,158,11,.04)");
  var gD = ctx.createLinearGradient(0,0,0,260); gD.addColorStop(0,"rgba(239,68,68,.35)"); gD.addColorStop(1,"rgba(239,68,68,.04)");
  var fills=[gA,gB,gC,gD];

  var datasets=[];
  data.series.forEach(function(s,i){
    var cfg = makeArea(s.color, fills[i%4], s.type==="pct"?"yL":"yR");
  ...
  CH_WE = new Chart(ctx,{ type:"line", data:{ labels:data.labels, datasets:datasets }, options:{
    ...
    scales:{
      x:{ grid:{display:false}, ticks:{ autoSkip:false, maxRotation:90, minRotation:90,
        callback:function(v,i){ return (i===0||i===data.labels.length-1)?"":data.labels[i]; } } },
      yL:{ position:"left", beginAtZero:true, ticks:{ callback:function(v){ return v+"%"; } } },
      yR:{ position:"right", beginAtZero:true, grid:{display:false}, ticks:{ callback:function(v){ return nice(v); } } }
    }
  }});
  var flat=[];
  datasets.forEach(function(d){ if(Array.isArray(d.data)) flat=flat.concat(d.data.slice(1,-1)); });
...
function buildDD9(){ buildDropdown("dd9menu", WE_MET.map(function(m){return m.k;}), WE_MET[0].k); wireDropdown("dd9"); }
function refreshWE(){
  var s = chosen("dd9menu");
  var tags = $("dd9tags"); tags.innerHTML="";
  s.forEach(function(k){ var t=document.createElement("span"); t.className="tag"; t.textContent=k; tags.appendChild(t); });
...
async function loadWE(){
  ...
  try{
    var csv = await fetchCSV(CSV_WE);
    RAW_WE = Papa.parse(csv,{header:false,skipEmptyLines:true}).data;
    $("t9time").textContent = new Date().toLocaleString();
    buildDD9(); refreshWE();
    qsa("#dd9menu input").forEach(function(i){ i.addEventListener("change", refreshWE); });
  }catch(e){
    console.error(e); $("t9err").textContent="Gagal memuatkan CSV (Warga Emas)."; $("t9err").style.display="block";
  }
}
$("t9refresh").addEventListener("click", loadWE);
$("t9expand").addEventListener("click", function(){
  if (!RAW_WE) return;
...
function safeChart(id){
  ...
  var labels=["","Kota Setar","Pendang","Kuala Muda","Sik","Kulim","Bandar Baru","Kubang Pasu","Pdg Terap","Baling","Yan","Langkawi","Kedah",""];
  var data=new Array(labels.length).fill(0); data[0]=0; data[data.length-1]=0;
  new Chart(ctx,{type:"line",data:{labels:labels,datasets:[{label:"Menunggu data",data:data,borderDash:[4,4],borderColor:"#94a3b8",pointRadius:0}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},
      scales:{x:{grid:{display:false},ticks:{autoSkip:false,maxRotation:90,minRotation:90,
        callback:function(v,i){ return (i===0||i===labels.length-1)?"":labels[i]; }}},y:{beginAtZero:true}}});
}
function initSafe(){
  ["t1","t2","t3","t4","t5","t6"].forEach(function(t){
    try{
      safeChart(t);
      var timeId = t + "time";
...
["t1refresh","t2refresh","t3refresh","t4refresh","t5refresh","t6refresh"].forEach(function(id){
  var el=$(id); if(el) el.addEventListener("click", initSafe);
});
["t1expand","t2expand","t3expand","t4expand","t5expand","t6expand"].forEach(function(id){
  var el=$(id); if(el) el.addEventListener("click", function(){ openModal("Paparan Besar"); safeChart("mcanvas"); });
});
