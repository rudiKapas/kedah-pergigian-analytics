...
async function fetchCSV(url){ const r = await fetch(url,{cache:"no-store"}); if(!r.ok) throw new Error("fetch csv"); return r.text(); }
function nice(n){ return (n==null||isNaN(n)) ? "0" : Number(n).toLocaleString(); }

/* A1 cell helpers */
function a1Cell(arr, A1){
  const m = /^([A-Z]+)(\d+)$/.exec(A1); if(!m) return "";
  let col = 0; for(const ch of m[1]) col = col*26 + (ch.charCodeAt(0)-64);
  const r = parseInt(m[2],10)-1, c = col-1;
  return (arr[r] && arr[r][c]) ?? "";
}
const valPct = (v)=>{ if(typeof v==="number") return v; if(!v) return 0; const x=String(v).replace(/[%\s]/g,""); const n=parseFloat(x); return isNaN(n)?0:n; };
const valInt = (v)=>{ if(typeof v==="number") return v; if(!v) return 0; const x=String(v).replace(/[^\d.-]/g,""); const n=parseInt(x,10); return isNaN(n)?0:n; };

/* dropdowns (icon-chip triggers) */
function buildDropdown(menuId, items, defaultKey){
  const menu = $(menuId);
  const body = qs(".menu-body", menu); body.innerHTML = "";
  items.forEach(k=>{
...
function chosen(menuId){
  const set = new Set();
  qsa(`#${menuId} .menu-body input[type=checkbox]`).forEach(cb=>{ if(cb.checked) set.add(cb.value); });
  if(set.size===0){
    const first = qs(`#${menuId} .menu-body input[type=checkbox]`); if(first){ first.checked=true; set.add(first.value); }
  }
  return set;
}
...
/* chart helpers */
function makeArea(color, fill, y="y", w=3){ return {borderColor:color, backgroundColor:fill, borderWidth:w, tension:.45, fill:true, yAxisID:y}; }
function makeDash(color, y="y"){ return {borderColor:color, borderWidth:2, borderDash:[5,5], pointRadius:0, fill:false, tension:0, yAxisID:y}; }
function allZero(arr){ return arr.every(v=> (Array.isArray(v)?allZero(v): (Number(v)||0)===0)); }
...
function computeYA(arr, set){
  const labels=["",...DIST.map(d=>d.n),""];
  const series=[];
  YA_MET.forEach(m=>{
    if(!set.has(m.k)) return;
    const data=[0]; DIST.forEach(d=> data.push(valPct(a1Cell(arr, d.L+String(m.row)))) ); data.push(0);
    series.push({label:m.k, data, color:m.color, target:m.target});
  });
...
function drawYA(data, canvasId){
  ...
  data.series.forEach((s,i)=>{
    datasets.push({...makeArea(s.color, i?g2:g1,"yL"), label:s.label, data:s.data});
    if(s.target!=null) datasets.push({...makeDash("#475569","yL"), label:`Sasaran ${s.label}`, data:new Array(s.data.length).fill(s.target)});
  });
  CH_YA = new Chart(ctx,{type:"line",data:{labels:data.labels,datasets},options:{
  ...
  if(allZero(datasets.flatMap(d=>d.data.slice?.(1,-1)??[]))){ ctx.font="12px Inter"; ctx.fillStyle="#94a3b8"; ctx.fillText("Tiada data untuk dipaparkan",12,22); }
}
function buildDD7(){ buildDropdown("dd7menu", YA_MET.map(m=>m.k), YA_MET[0].k); wireDropdown("dd7"); }
function refreshYA(){ const s=chosen("dd7menu"); const tags=$("dd7tags"); tags.innerHTML=""; s.forEach(k=>{const t=document.createElement("span");t.className="tag";t.textContent=k;tags.appendChild(t)}); if(RAW_YA) drawYA(computeYA(RAW_YA,s),"t7"); }
async function loadYA(){
  $("t7err").style.display="none";
  try{ const csv=await fetchCSV(CSV_YA); RAW_YA=Papa.parse(csv,{header:false,skipEmptyLines:true}).data;
    $("t7time").textContent=new Date().toLocaleString(); buildDD7(); refreshYA();
...
$("t7expand").addEventListener("click", ()=>{ if(!RAW_YA) return; openModal("Young Adult"); MCH=new Chart($("mcanvas").getContext("2d"), CH_YA.config); });
...
function computeBPE(arr, set){
  ...
  BPE_MET.forEach(m=>{
    if(!set.has(m.k)) return;
    const data=[0]; DIST.forEach(d=> data.push(valPct(a1Cell(arr, d.L+String(m.row)))) ); data.push(0);
    series.push({label:m.k, data, color:m.color});
  });
  return {labels, series};
}
function drawBPE(data, canvasId){
  ...
  const datasets = data.series.map((s,i)=> ({...makeArea(s.color, i?g2:g1, "yL"), label:s.label, data:s.data}));
  CH_BPE = new Chart(ctx,{type:"line",data:{labels:data.labels,datasets},options:{
    responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},
      tooltip:{mode:"index",intersect:false,filter:(i)=>!(i.dataIndex===0||i.dataIndex===data.labels.length-1)}},
    scales:{x:{grid:{display:false},ticks:{autoSkip:false,maxRotation:90,minRotation:90,callback:(v,i)=>(i===0||i===data.labels.length-1)?"":data.labels[i]}},
            yL:{position:"left",beginAtZero:true,ticks:{callback:(v)=>v+"%"}}}});
...
function refreshBPE(){ const s=chosen("dd8menu"); const tags=$("dd8tags"); tags.innerHTML=""; s.forEach(k=>{const t=document.createElement("span");t.className="tag";t.textContent=k;tags.appendChild(t)}); if(RAW_BPE) drawBPE(computeBPE(RAW_BPE,s),"t8"); }
async function loadBPE(){
  $("t8err").style.display="none";
  try{ const csv=await fetchCSV(CSV_BPE); RAW_BPE=Papa.parse(csv,{header:false,skipEmptyLines:true}).data;
    $("t8time").textContent=new Date().toLocaleString(); buildDD8(); refreshBPE();
...
$("t8expand").addEventListener("click", ()=>{ if(!RAW_BPE) return; openModal("Basic Periodontal Examination"); MCH=new Chart($("mcanvas").getContext("2d"), CH_BPE.config); });
...
function computeWE(arr, set){
  ...
  WE_MET.forEach(m=>{
    if(!set.has(m.k)) return;
    const data=[0]; DIST.forEach(d=> data.push(m.type==="pct" ? valPct(a1Cell(arr, d.L+String(m.row))) : valInt(a1Cell(arr, d.L+String(m.row)))) ); data.push(0);
    series.push({label:m.k, data, color:m.color, target:m.target, type:m.type});
  });
  return {labels, series};
}
function drawWE(data, canvasId){
  ...
  const datasets=[];
  data.series.forEach((s,i)=>{
    datasets.push({...makeArea(s.color, fills[i%4], s.type==="pct"?"yL":"yR"), label:s.label, data:s.data});
    if(s.target!=null && s.type==="pct") datasets.push({...makeDash("#475569","yL"), label:`Sasaran ${s.label}`, data:new Array(s.data.length).fill(s.target)});
  });

  CH_WE = new Chart(ctx,{type:"line",data:{labels:data.labels,datasets},options:{
    responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},
      tooltip:{mode:"index",intersect:false,filter:(i)=>!(i.dataIndex===0||i.dataIndex===data.labels.length-1)}},
    scales:{x:{grid:{display:false},ticks:{autoSkip:false,maxRotation:90,minRotation:90,callback:(v,i)=>(i===0||i===data.labels.length-1)?"":data.labels[i]}},
            yL:{position:"left",beginAtZero:true,ticks:{callback:(v)=>v+"%"}},
...
function refreshWE(){ const s=chosen("dd9menu"); const tags=$("dd9tags"); tags.innerHTML=""; s.forEach(k=>{const t=document.createElement("span");t.className="tag";t.textContent=k;tags.appendChild(t)}); if(RAW_WE) drawWE(computeWE(RAW_WE,s),"t9"); }
async function loadWE(){
  $("t9err").style.display="none";
  try{ const csv=await fetchCSV(CSV_WE); RAW_WE=Papa.parse(csv,{header:false,skipEmptyLines:true}).data;
    $("t9time").textContent=new Date().toLocaleString(); buildDD9(); refreshWE();
...
$("t9expand").addEventListener("click", ()=>{ if(!RAW_WE) return; openModal("Warga Emas"); MCH=new Chart($("mcanvas").getContext("2d"), CH_WE.config); });
...
function safeChart(id){
  ...
  new Chart(ctx,{type:"line",data:{labels,datasets:[{label:"Menunggu data",data,borderDash:[4,4],borderColor:"#94a3b8",pointRadius:0}]},
...
function initSafe(){
  ["t1","t2","t3","t4","t5","t6"].forEach(t=>{ try{ safeChart(t); const timeId = t + "time"; if($(timeId)) $(timeId).textContent=new Date().toLocaleString(); }catch(e){} });
}
["t1refresh","t2refresh","t3refresh","t4refresh","t5refresh","t6refresh"].forEach(id=>{ const el=$(id); if(el) el.addEventListener("click", initSafe); });
["t1expand","t2expand","t3expand","t4expand","t5expand","t6expand"].forEach(id=>{ const el=$(id); if(el) el.addEventListener("click", ()=>{ openModal("Paparan Besar"); safeChart("mcanvas"); }); });
